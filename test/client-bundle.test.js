...
test('客户端 Bundle 实时刷新链路完整性测试', async (t) => {
  await t.test('useEffect 不得在 locale 订阅处提前 return，SSE 监听与轮询必须注册', () => {
    const localeEffect = bundle.slice(bundle.indexOf("监听 DSH locale 服务动态切换"))
    ...
    // 3. 设备事件监听必须注册
    assert.ok(localeEffect.includes("addEventListener('dsh-device-updated', handleDeviceUpdate)"))
    // 4. 3 秒轮询必须启动
    assert.ok(localeEffect.includes('setInterval'))
    assert.ok(localeEffect.includes('refreshStatusOnly()'))
    // 5. cleanup 中必须同时清理 localStorage 数组、监听器与轮询
    assert.ok(localeEffect.includes('clearInterval(pollTimer)'))
    assert.ok(localeEffect.includes("removeEventListener('dsh-device-updated'"))
